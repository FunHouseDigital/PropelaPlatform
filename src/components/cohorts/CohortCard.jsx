...
import { getCohorts, saveCohorts } from '../../lib/storage';
import { MAX_LENGTHS, sanitizeText } from '../../lib/validation';
...
export default function CohortCard({ cohort, onClose, onUpdate }) {
  ...
  const cohortNurses = useMemo(
    () => nurses.filter((nurse) => nurse.cohortAssigned === data.name),
    [data.name, nurses]
  );
  ...
  const commitmentFeesCollected =
    cohortNurses.filter((n) => n.commitmentFeeStatus === 'Paid').length * 2500; // R2,500 per nurse

  const totalSpent =
    (data.budget.trainingCostActual || 0) +
  ...
  const oetAttempted = cohortNurses.filter((n) =>
    ['Passed', 'Failed'].includes(n.oetStatus)
  ).length;
  const oetPassRateActual =
    oetAttempted > 0 ? Math.round((oetPassedCount / oetAttempted) * 100) : 0;

  const placedCount = cohortNurses.filter((n) => n.pipelineStage === 'Placed').length;
  const placementRateActual =
    enrolledCount > 0 ? Math.round((placedCount / enrolledCount) * 100) : 0;
        ...
        <div className="divide-y divide-gray-100">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateField('trainingProvider.costPerNurse', parseInt(e.target.value) || 0)
                  }
                  className="w-full text-sm border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-propela-purple"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateField('trainingProvider.lessonsPlanned', parseInt(e.target.value) || 0)
                  }
                  className="w-full text-sm border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-propela-purple"
                />
              </div>
          ...
          <Section title="Budget" icon={DollarSign}>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-xs font-medium text-gray-500 block mb-1">
                  Propela Contribution
                </label>
                <input
                  type="number"
                  value={data.budget?.propelaContribution || ''}
                  onChange={(e) =>
                    updateField('budget.propelaContribution', parseInt(e.target.value) || 0)
                  }
                  className="w-full text-sm border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-propela-purple"
                />
              </div>
              <div>
                <label className="text-xs font-medium text-gray-500 block mb-1">
                  Commitment Fees Collected (auto)
                </label>
                <div className="text-sm font-medium text-gray-900 px-3 py-1.5 bg-gray-50 rounded-lg">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateField('budget.trainingCostActual', parseInt(e.target.value) || 0)
                  }
                  className="w-full text-sm border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-propela-purple"
                />
              </div>
              <div>
                <label className="text-xs font-medium text-gray-500 block mb-1">
                  OET Exam Cost Actual
                </label>
                <input
                  type="number"
                  value={data.budget?.oetExamCostActual || ''}
                  onChange={(e) =>
                    updateField('budget.oetExamCostActual', parseInt(e.target.value) || 0)
                  }
                  className="w-full text-sm border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-propela-purple"
                />
              </div>
              ...
              <div>
                <label className="text-xs font-medium text-gray-500 block mb-1">
                  Budget Remaining (auto)
                </label>
                <div
                  className={`text-sm font-medium px-3 py-1.5 rounded-lg ${budgetRemaining >= 0 ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}
                >
          ...
          <Section title="Outcomes" icon={Target}>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-medium text-gray-500 block mb-1">
                  OET Pass Rate Target (%)
                </label>
                <input
                  type="number"
                  value={data.outcomes?.oetPassRateTarget || 80}
                  onChange={(e) =>
                    updateField('outcomes.oetPassRateTarget', parseInt(e.target.value) || 0)
                  }
                  className="w-full text-sm border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-propela-purple"
                />
              </div>
              <div>
                <label className="text-xs font-medium text-gray-500 block mb-1">
                  OET Pass Rate Actual (auto)
                </label>
                <div
                  className={`text-sm font-medium px-3 py-1.5 rounded-lg ${oetPassRateActual >= (data.outcomes?.oetPassRateTarget || 80) ? 'bg-green-50 text-green-700' : 'bg-amber-50 text-amber-700'}`}
                >
                  {oetPassRateActual}%
                </div>
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateField('outcomes.placementRateTarget', parseInt(e.target.value) || 0)
                  }
                  className="w-full text-sm border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-propela-purple"
                />
              </div>
              <div>
                <label className="text-xs font-medium text-gray-500 block mb-1">
                  Placement Rate Actual (auto)
                </label>
                <div
                  className={`text-sm font-medium px-3 py-1.5 rounded-lg ${placementRateActual >= (data.outcomes?.placementRateTarget || 70) ? 'bg-green-50 text-green-700' : 'bg-amber-50 text-amber-700'}`}
                >
                  {placementRateActual}%
                </div>
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateField('outcomes.totalPlacementFees', parseInt(e.target.value) || 0)
                  }
                  className="w-full text-sm border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-propela-purple"
                />
              </div>
          ...
          <Section
            title={`Nurses (${cohortNurses.length})`}
            icon={GraduationCap}
            defaultOpen={true}
          >
