import { Calendar, DollarSign, GraduationCap, Plus, Users } from 'lucide-react';
...
export default function CohortManager() {
  const { nurses: allNurses } = useAppContext();
  ...
  return (
    <div>
      ...
      <p className="text-gray-500 text-sm mb-8">
        Track each cohort as a discrete delivery project, from planning through to final placement
        outcomes and documented learnings.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {cohorts.map((cohort) => {
          ...
          const totalSpent =
            (cohort.budget?.trainingCostActual || 0) +
          ...
          return (
            ...
            >
              ...
              <div className="flex items-start justify-between mb-3">
                ...
                <span
                  className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${getStatusColor(cohort.status)}`}
                >
                  {cohort.status}
                </span>
              </div>
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-gray-500">
                  {cohort.trainingStart ? formatDate(cohort.trainingStart) : 'TBD'} -{' '}
                  {cohort.trainingEnd ? formatDate(cohort.trainingEnd) : 'TBD'}
                </span>
              </div>
              ...
              <div className="mt-3">
                <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${cohort.targetNurses > 0 ? Math.min((enrolled / cohort.targetNurses) * 100, 100) : 0}%`,
                    }}
                  />
                </div>
              </div>
